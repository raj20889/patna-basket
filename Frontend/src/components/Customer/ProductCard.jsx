...
const RelatedProducts = ({ products, onCartUpdate, cart: parentCart, loading: parentLoading }) => {
  ...
  const handleChange = async (productId, change) => {
    ...
    try {
      // FIXED: Pass newQty instead of newQuantity
  ...
  const dairyProducts = (Array.isArray(products) ? products : []).filter(product => {
    ...
    return category.includes('milk') || category.includes('dairy') || 
           category.includes('bread') || category.includes('bakery') ||
           category.includes('egg') || name.includes('milk') || 
           name.includes('dairy') || name.includes('bread') || 
           name.includes('bakery') || name.includes('egg');
  });
