...
const AddCategoryModal = ({ isOpen, onClose, onAdd, loading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: messages.category.addSuccess });
      setTimeout(() => {
        setFormData({ name: '', description: '', image: '' });
        onClose();
      }, 500);
    } catch (err) {
      setToast({ type: 'error', message: messages.category.addError });
    } finally {
  ...
  return (
    <>
      <Modal isOpen={isOpen} title="Create New Category" onClose={onClose} size="md">
