...
const AddSubcategoryModal = ({ isOpen, onClose, onAdd, categories, loading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: messages.subcategory.addSuccess });
      setTimeout(() => {
        setFormData({ name: '', category: '', description: '', image: '' });
        onClose();
      }, 400);
    } catch (err) {
      setToast({ type: 'error', message: messages.subcategory.addError });
    } finally {
  ...
  return (
    <>
      <Modal isOpen={isOpen} title="Create Subcategory" onClose={onClose} size="md">
